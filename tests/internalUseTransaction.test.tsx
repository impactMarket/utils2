...
it('internalUseTransaction hook runs correctly', async () => {
    const cusdMock = { balanceOf: jest.fn(() => Promise.resolve(toToken(0.02))) };
    const connectionMock = {
        estimateGas: jest.fn(() => Promise.resolve(10)),
        gasPrice: jest.fn(() => Promise.resolve(1)),
        sendTransaction: jest.fn(() => Promise.resolve({ waitReceipt: jest.fn(() => Promise.resolve(true)) }))
    };
    const getContractsMock = jest.spyOn(contracts, 'getContracts');
